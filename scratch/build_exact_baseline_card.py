...
print("=== BUILDING EXACT BASELINE CARD FOR JULY 18/19 2026 WITH REAL XGBOOST V3 INFERENCE ===")

model, raw_df, medians, all_fighters, model_path_used = load_resources_v3()
print(f"Loaded calibrated XGBoost V3 model from: {model_path_used}")
...
for idx, item in enumerate(official_baseline_fights):
    ...
    st_val = f"VB: {is_value_bet} ({bet_fighter} @ {bet_odds})" if is_value_bet else "NO BET"
    res_str = f"{res_status} ({net_gain:+.2f}€)" if res_status else "N/A"
    print(f"  [{idx+1:02d}] {f1_raw} ({odds_a}) vs {f2_raw} ({odds_b}) | IA %: {pct_a:.1f}% / {pct_b:.1f}% | {st_val:<32} | Res: {res_str}")
...
print("\n" + "="*80)
print(f"EXACT BASELINE CARD BUILT AND SAVED IN data/historical_tracker.json")
print(f"Total Unique Fights: {len(fights_list)} (Strictly 12 fights, 0 duplicates)")
print(f"Real XGBoost V3 Financial Metrics: Profit={total_profit:+.2f}€ | ROI={roi_pct:+.1f}% | WinRate={win_rate_pct:.1f}% ({value_bets_won}/{value_bets_count}) | Volume={total_staked:.0f}€")
print("="*80)
